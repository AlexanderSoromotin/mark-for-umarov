...
if ($record_data -> num_rows != 0) {
	$record = mysqli_fetch_assoc($record_data);
	$record_subsection_id = $record['group_id'];
	$record_subsection_data = mysqli_fetch_assoc(mysqli_query($connection, "SELECT * FROM `interests_subsections` WHERE `id` = '$record_subsection_id'"));
	$record_subsection_title = $record_subsection_data[$lang . '_title'];
	$record_title = $record['title'];
	$record_body = $record['body'];
	$record_loc = $record['loc'];
	$record_city_id = $record['city_id'];
	$record_repost = $record['repost'];
...
<body>
	...
	?>
		...
		<? if ($record_id == '' or $record_data -> num_rows == 0) : ?>
		...
		<!-- Хронология -->
			<div class="history">
				<div class="block">
					...
					<a href="<?$link?>/interest-groups">Группы интересов</a>
					<img draggable="false" src="<? $link ?>/assets/img/icons/chevron-right.svg">
					<a href="<?= $link ?>/interest-groups/?subsections=<?= $record_subsection_id ?>"><?= $record_subsection_title ?></a>
				</div>
			</div>

			<?
				$record = mysqli_fetch_assoc(mysqli_query($connection, "SELECT * FROM `interests_records` WHERE `id` = '$record_id'"));
				$record_user_id = $record['user_id'];
				$local_user_data = mysqli_fetch_assoc(mysqli_query($connection, "SELECT * FROM `users` WHERE `id` = '$record_user_id'"));

			?>
		
			<div class="main">
				<div class="messageList">
					<div class="firstPost post" alt="user_<?= $record_user_id ?>">
						<a href="<?= $link ?>/interest-groups/user/?edit=<?= $record['id'] ?>">
						...
						<div class="name">
							<a href="<?= $link ?>/profile/?id=<?= $user_id ?>">
								...
								<div class="bullet-point"></div><div class="date"><?= calcTime($record['date']) ?></div>

								<? if ($record['last_redactor'] != 0) : ?>
					...
					<div class="replies">
						...
						<?
							if ($record['replies'] != '') {
								$replies = unserialize($record['replies']);
								...
								if (count($replies) != 0) {
									foreach ($replies as $reply_id => $reply_data) {
										...
										$author = '';
										if ($record['user_id'] == $reply_data['user_id']) {
