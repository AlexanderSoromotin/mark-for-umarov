...
<body>
	...
	<div class="main">
		...
		<div class="chats mobile_deactive_block">
	...
	<script type="text/javascript">
		...
		function addFile (local_chat_id, files, compress) {
			...
			console.log('Загрузка файлов в чат', local_chat_id)
				...
				var addFileInterval = setInterval(function () {
					if (i < files.length && files[i]['size'] > 0) {
						...
						if (files[i].size / 1024 / 1024 > 50) {
							alert('Размер файла "' + files[i].name + '" превышает 50мб');
							i++;
							return;
						}

						if (files_list[local_chat_id] == undefined) {
							files_list[local_chat_id] = [];
						}

						if( typeof files[i] == 'undefined' ) return;

						// Создание прогрессбара
						$('#chat_' + local_chat_id + ' .input-form label').text('').append('<div class="progress_bar"><div class="progress_line"></div></div><p class="file_name">' + files[i].name.substr(0, 35) + '</p>')
						
						// Создание счётчика количества файлов
						if ($('#chat_' + local_chat_id + ' .input .upload-files-info').length == 0) {
							$('#chat_' + local_chat_id + ' .input').append('<div class="upload-files-info">Файлов: <b>1</b> <button class="button-3 delete_all_media_files">Удалить все</button></div>')
						}

						// Анимация появления счётчика файлов
						$('#chat_' + local_chat_id + ' .upload-files-info').addClass('upload-files-info-display');
						...
						$.ajax({
							...
							xhr: function(){
						        ...
						        xhr.upload.addEventListener('progress', function (e) {
						        	if (e.lengthComputable) {
						            	...
						            	$('#chat_' + local_chat_id + ' .input-form label .progress_line').css({"width" : percentComplete + '%'})
						        	}
						        }, false);
						        return xhr;
						    }, 
							success : function(result) {
								...
								if (result != '') {
									...
									files_list[local_chat_id].push(files_i);
									
									// По какой-то причине после цикла for значение chat_id изменяется, для это введено tmp_local_chat_id
									let tmp_local_chat_id = local_chat_id;

									// Добавление к файлам, находящимся в списке файлов дополнительных данных
									for (tmp_local_chat_id in files_list) {
										for (file_index in files_list[tmp_local_chat_id]) {
											object = files_list[tmp_local_chat_id][file_index];
											localObject = new Object();

											if (object['file_id'] == result['file_id']) {
												localObject = object;
												files_list[tmp_local_chat_id][file_index] = new Object();

												// console.log(object['name'])
												// console.log(result['stock_name'])

												files_list[tmp_local_chat_id][file_index]['server_name'] = result['name'];
												files_list[tmp_local_chat_id][file_index]['name'] = result['stock_name'];
												files_list[tmp_local_chat_id][file_index]['file_id'] = fileId;
												files_list[tmp_local_chat_id][file_index]['file_type'] = result['file_type'];
												files_list[tmp_local_chat_id][file_index]['mime'] = result['mime'];
												files_list[tmp_local_chat_id][file_index]['size'] = localObject['size'];
											}
										}
									}

									// Изменение данных в локальном хранилище
									if (localStorage.getItem('saved_chat_data_' + local_chat_id) == '' || localStorage.getItem('saved_chat_data_' + local_chat_id) == null) {
										localStorage.setItem('saved_chat_data_' + local_chat_id, '{"message":"", "media": []}');
									}

									saved_chat_data = JSON.parse(localStorage.getItem('saved_chat_data_' + local_chat_id));

									saved_chat_data['media'] = [];
									for (file_index in files_list[local_chat_id]) {
										file = files_list[local_chat_id][file_index];
									...
									localStorage.setItem('saved_chat_data_' + local_chat_id, JSON.stringify(saved_chat_data));

									// Добавление файла в файлбар под полем ввода сообщения
									if (result['file_type'] == 'img') {
										$('#chat_' + local_chat_id + ' .uploaded-files').append("<div title='" + result['stock_name'] + "' class='media_file hidden_media_file'><img src='" + result['url'] + "'><div class='file_name'>" + result['stock_name'] + "</div><img class='delete_media_file' alt='file_" + fileId + "' src='<?= $link ?>/assets/img/icons/x.svg'></div>");
										setTimeout(function () {
											$('.media_file').removeClass('hidden_media_file')
											$('#chat_' + local_chat_id + ' .input .upload-files-info b').text($('.uploaded-files .media_file').length);
										}, 300)
									} else {
										$('#chat_' + local_chat_id + ' .uploaded-files').append("<div title='" + result['stock_name'] + "' class='media_file hidden_media_file'><div class='file_background'>" + result['mime'] + "</div><div class='file_name'>" + result['stock_name'] + "</div><img class='delete_media_file' alt='file_" + fileId + "' src='<?= $link ?>/assets/img/icons/x.svg'></div>");
										setTimeout(function () {
									...
									if (i == (files.length)) {
										...
										$('#chat_' + local_chat_id + ' .input-form label .progress_bar').remove();
										$('#chat_' + local_chat_id + ' .input-form label').text('');
										$('#chat_' + local_chat_id + ' .input-form label').append('Выберите <b>файл</b> или перетащите его сюда');

										// Если загруженный файл является последний, то поле сбрасывания файлов скрывается
										$('#chat_' + local_chat_id + ' .input textarea').css({"display" : "flex"})
										$('#chat_' + local_chat_id + ' .input .upload-files').css({"display" : "none"});
										$('#chat_' + local_chat_id + ' .input .add-media-file').css({"display" : "flex"})	
									}

									// Фокус на поле ввода
									textarea_val = $('.main .chats #chat_' + local_chat_id + ' .input textarea').val();
									$('.main .chats #chat_' + local_chat_id + ' .input textarea').val('').focus().val(textarea_val);
						...
						files_sending[local_chat_id] = 0;
						$('#chat_' + local_chat_id + ' .input input').val('');
		...
		$('body').on('click', '.delete_all_media_files', function () {
			...
			local_chat_id = $(this).parent().parent().parent().attr('id').replace('chat_', '');
			delete(files_list[local_chat_id]);
			...
			$('#chat_' + local_chat_id + ' .uploaded-files .media_file').addClass('hidden_media_file');
			$('#chat_' + local_chat_id + ' .upload-files-info').removeClass('upload-files-info-display');

			setTimeout(function () {
				$('#chat_' + local_chat_id + ' .uploaded-files .media_file').remove();
			}, 300)
		})
		...
		$(document).on('change', '.input input', function () {
			...
			console.log('Загрузка файла из input', $(this).parents('.chats-block').attr('id').replace('chat_', ''));
		})
...
// 		})
		...
		$(document).on('click', '.add-media-file', function () {
			// $('.opened_chat .input textarea').css({"display" : "none"})
			// $('.opened_chat .input .add-media-file').css({"display" : "none"})
			// $('.opened_chat .input .upload-files').css({"display" : "flex"})
		...
		$(document).on("drop", ".chats-block .input-form label", function(e) {
			...
			console.log('Дроп файла', chat_id);
		});
		...
		function openChat (user_id) {
			...
			$('.users').addClass('mobile_deactive_block');
			$('.chats').removeClass('mobile_deactive_block');
			$('.mobile_footer').addClass('mobile_deactive_block');
			$('.header').addClass('mobile_deactive_block');
		}

		// Сворачивание окна диалога
		$(document).on('click', '.back_to_users_list', function () {
			$('.users').removeClass('mobile_deactive_block');
			$('.chats').addClass('mobile_deactive_block');
			$('.mobile_footer').removeClass('mobile_deactive_block');
			$('.header').removeClass('mobile_deactive_block');
			$('.chats-block').removeClass('opened_chat')
		})
			...
			success: function (result) {
				...
				for (let eq = 0; eq < chats_length; eq++) {
					...
					if (chat_id != '' && chat_id != 'chat_start') {
						...
						if (temporary_storage != '' && temporary_storage != null) {
							...
							$('#chat_' + chat_id + ' .input textarea').css({'height' : height + ' !important'});
						} else {
				...
				$('.input textarea').bind('textarea paste', function(e) {
		      		...
		      		if (e.clipboardData.files.length != 0) {
				    	...
				    	console.log('Вставка файла', $(this).parents('.chats-block').attr('id').replace('chat_', ''));
		      		}
		...
		function addMessage(id, message) {
			...
			$('#user_' + id + ' .last_message_time').text(date.getHours() + ':' + date.getMinutes());
			...
			if ($('.main #chat_' + id + ' .message_block:eq(0)').hasClass('incoming_msg')) {
				...
				if (message_text.length == 0) {
					$('.main #user_' + id + ' p:eq(1)').text('Вы: ').prepend('<b class="media">Вложение</b>');
				} else {
					if (message_text.length > 75) {
				...
				if ($('.main #user_' + id).hasClass('unread')) {
					$('.main #user_' + id).removeClass('unread');
				}
	...
	<script type="text/javascript">
		select_mobile_footer_tab('messenger');

		if (BrowserDetect.browser == 'Chrome' && BrowserDetect.OS == 'Linux') {
			$('head').append('<link rel="stylesheet" href="mobile_chrome.css">');
		}
		if (BrowserDetect.browser == 'Safari' && BrowserDetect.OS == 'iPhone/iPod') {
			$('head').append('<link rel="stylesheet" href="mobile_safari.css">');
		}
